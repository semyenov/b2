...
export function createEmptyBoard(size: number): Letter[][] {
  ...
  for (let r = 0; r < size; r++) {
    const row: Letter[] = new Array(size).fill(null);
    board.push(row);
  }
  return board;
}
...
export function normalizeLetter(letter: string): string {
  if (!letter || letter.length === 0) return letter;
...
const ORTHOGONAL_DIRS: BoardPosition[] = [
  { row: -1, col: 0 },
  { row: 1, col: 0 },
  { row: 0, col: -1 },
  { row: 0, col: 1 }
...
export function isAdjacentToExisting(board: Letter[][], pos: BoardPosition): boolean {
  ...
  forEachNeighbor(board.length, pos, (n) => {
    if (board[n.row][n.col] !== null) adjacent = true;
  });
  return adjacent;
}
...
export function existsPathForWord(
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      if (board[r][c] === first) starts.push({ row: r, col: c });
    }
  }
  ...
  const dfs = (pos: BoardPosition, index: number): boolean => {
    ...
    if (cell !== target[index]) return false;
    visited.add(key);
    try {
      ...
      forEachNeighbor(size, pos, (n) => {
        if (found) return;
        if (board[n.row][n.col] == null) return;
        if (board[n.row][n.col] !== target[index + 1]) return;
        if (dfs(n, index + 1)) found = true;
      });
      return found;
    } finally {
      visited.delete(key);
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      ...
      for (const letter of candidateLetters) {
        ...
        board[r][c] = null;
        if (ok) results.push({ position: pos, letter });
      }
    }
  }
  return results;
