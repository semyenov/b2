...
/**
 * Normalize letter with configuration-based letter mapping
 * @param letter - Letter to normalize
 * @param letterNormalization - Letter normalization configuration
 * @param letterNormalization.normalizeYoToE - Whether to normalize Ё to Е
 * @param letterNormalization.normalizeEToYo - Whether to normalize Е to Ё
 * @returns Normalized letter
 */
export function normalizeLetterWithConfig(
  letter: string,
  letterNormalization: { normalizeYoToE: boolean, normalizeEToYo: boolean },
): string {
  if (!letter || letter.length === 0)
    return letter
  const firstChar = letter[0]
  ...
  let normalized = firstChar.toUpperCase()

  // Apply letter normalization based on configuration
  if (letterNormalization.normalizeYoToE && normalized === 'Ё') {
    normalized = 'Е'
  }
  else if (letterNormalization.normalizeEToYo && normalized === 'Е') {
    normalized = 'Ё'
  }
...
/**
 * Normalize word with configuration-based letter mapping
 ...
 */
export function normalizeWordWithConfig(
  word: string,
  letterNormalization: { normalizeYoToE: boolean, normalizeEToYo: boolean },
): string {
  ...
  return trimmed
    .toUpperCase()
    .split('')
    .map(letter => normalizeLetterWithConfig(letter, letterNormalization))
    .join('')
}
...
export function applyMove(
  ...
  const { position } = move
  const letter = letterNormalization
    ? normalizeLetterWithConfig(move.letter, letterNormalization)
    : normalizeLetter(move.letter)
  const word = letterNormalization
    ? normalizeWordWithConfig(move.word, letterNormalization)
    : normalizeWord(move.word)
...
export function createGame(
  ...
  const normalizedBaseWord = letterNormalization
    ? normalizeWordWithConfig(baseWord, letterNormalization)
    : normalizeWord(baseWord)
